...
export function useEditStatus() {
  ...
  const checkEditStatus = async () => {
    try {
      ...
      // Show notification if application is finalized
      if (!data.can_edit && data.finalized_at) {
        toast({
          title: "📋 Aplikasi Sudah Dikirim",
          description: "Aplikasi beasiswa Anda sudah dikirim dan tidak dapat diubah lagi.",
          variant: "default",
        })
      }
