...
export default function ManageAdminPage() {
  ...
  // Fungsi untuk memfilter data admin berdasarkan pencarian
  const filteredAdmins = admins.filter(admin => 
    admin.name.toLowerCase().includes(searchQuery.toLowerCase()) || 
    admin.email.toLowerCase().includes(searchQuery.toLowerCase()) ||
    admin.role.toLowerCase().includes(searchQuery.toLowerCase())
  ...
  const handleRoleChange = (value: string) => {
    setFormData(prev => ({
      ...
      role: value
        ...
        title: "Gagal memuat data admin",
        description: "Endpoint API untuk daftar user tidak tersedia. Hubungi developer untuk mengimplementasikannya.",
        variant: "destructive"
      });
      
      // Berikan instruksi tambahan untuk developer
      console.error(`
        DEVELOPER NOTE: 
        Endpoint yang diperlukan untuk halaman ini tidak tersedia.
        Perlu diimplementasikan di backend:
