...
export default function AdminDokumenPendukungPage() {
  ...
  const handlePreview = (doc: Document) => {
    ...
    // Log untuk debug
    console.log('=== 🔍 ADMIN PENDUKUNG PREVIEW DEBUG INFO ===')
    ...
    fetch(directFileUrl, { method: 'HEAD' })
      .then(response => {
        ...
        if (!response.ok) {
          console.warn('⚠️ URL might not be accessible:', directFileUrl)
        } else {
            ...
            {selectedDoc && (
              selectedDoc.file_path.toLowerCase().endsWith('.pdf') ? (
                <iframe 
                  ...
                  onLoad={() => {
                    console.log('✅ PDF loaded successfully for admin pendukung preview')
                  }}
                  onError={() => {
                    console.error('❌ Failed to load PDF in admin pendukung preview:', selectedDoc.file_path)
                  }}
                />
              ) : (
                <div className="flex items-center justify-center w-full h-full bg-gray-50">
                  <img 
                    ...
                    onLoad={() => {
                      console.log('✅ Image loaded successfully for admin pendukung preview')
                    }}
                    onError={(e) => {
                      console.error('❌ Failed to load image in admin pendukung preview:', selectedDoc.file_path)
                      ...
                      if (!target.src.includes('cacheBuster=')) {
                        ...
                        const baseUrl = selectedDoc.file_path.split('?')[0]
                        target.src = `${baseUrl}?cacheBuster=${cacheBuster}`
                        return
                      }
                      
                      // If still fails, show error message
                      console.warn('💥 Image load failed even with cache busting, showing fallback UI')
                      target.style.display = 'none'
                      
                      // Avoid creating multiple error divs
                      if (target.parentElement?.querySelector('.preview-error-message')) {
                        return
                      }
                      
                      // Create error message with download link
                      const errorDiv = document.createElement('div')
                      errorDiv.className = 'preview-error-message flex flex-col items-center justify-center h-full text-gray-500 p-8 text-center'
                      errorDiv.innerHTML = `
